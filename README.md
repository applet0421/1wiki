...
同一頁的「資料清理保留期限」可分別設定 LLM 用量、流量資料、搜尋通知、AI 配圖任務、快取失敗事件與備份失敗紀錄的保留天數，範圍為 1–3650 天。預設值為：LLM 180 天、流量頁面 365 天、流量網站 730 天、同步紀錄 180 天、搜尋成功 90 天、搜尋失敗 365 天、AI 配圖 90 天、快取失敗 180 天、備份失敗 30 天。`database-backup-worker` 每 24 小時最多執行一次清理；過期 Session 會清除，核心文章與設定資料不會自動刪除。仍在處理中的工作與仍含原始圖片的 AI 任務會受到保護。
