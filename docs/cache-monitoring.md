...
文件狀態：現行操作文件；本機流程已有實作，正式 Cloudflare zone、token、DNS、VM／Coolify 部署未由本次文件整理確認。
...
成功事件完成後會在 `PublicInvalidation` 留下 `SUCCESS` 與 `completedAt`，讓 `/admin/cache` 顯示最近成功時間；`PENDING` 不會自動刪除，`SUCCESS`／`FAILED` 則依 `/admin/database-backups` 的「資料清理保留期限」設定保留，預設 180 天。資料清理由 `database-backup-worker` 每 24 小時最多執行一次。
