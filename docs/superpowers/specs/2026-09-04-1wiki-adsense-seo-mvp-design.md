...
本文件保留原始 MVP 決策；目前程式已擴充多語系、三級分類、作者庫、GA4、AI 配圖與 Worker 監控、分類列表與文章連續閱讀、卡片封面及 YouTube Shorts。最新入口與狀態以 [README](../../../README.md) 和 [工作狀態](../../project-status.md) 為準。

`d5f0074` 新增封面上傳／發布補值與部分搜尋引擎通知。SEO 與封面行為見 [文章編輯](../../article-editing.md)；IndexNow 的交易、排程與完整驗收仍待補，見 [搜尋引擎通知](../../search-engine-submission.md)。下方原始資料模型、版位表與路由表是設計基準，完整現況須搭配上述增補文件；本次未重跑 MVP 全套驗收。
