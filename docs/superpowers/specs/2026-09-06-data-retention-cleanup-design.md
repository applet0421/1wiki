...
文件狀態：歷史資料保留設計規格；現況以 [工作狀態](../../project-status.md) 為準。

> 需求修訂（2026-09-06）：流量不需要日期趨勢或成長分析，改為每頁單筆累計瀏覽量；不再保存每日頁面與網站彙總。
...
管理者可在 `/admin/database-backups` 設定下列天數。設定值代表以資料的主要建立時間為基準，保留最近 N 天；最小值為 1，最大值為 3650。流量資料不保存每日頁面或網站彙總。

| 資料 | 預設保留 | 清理條件 |
|---|---:|---|
| LLM 用量 | 180 天 | 依 `createdAt` 清理成功與失敗紀錄 |
| 單頁累計瀏覽量 | 長期 | 每個 canonical `pagePath` 一筆，不做日期清理 |
