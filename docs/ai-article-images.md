...
文件狀態：現行操作文件；功能已有程式實作，真實 Gemini、R2 與正式 Worker 狀態須逐環境驗證。

文章編輯器「上傳圖片」旁的 AI 配圖按鈕，使用最新未儲存標題及正文。先分析出可修改的 Prompt、alt 草稿與插入段落，再生成單張圖片、上傳 R2，最後由編輯者預覽並插入正文。不自動儲存或發布文章。
