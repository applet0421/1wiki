...
| 公開 ISR、快取失效 Outbox、Cloudflare purge 與 `/admin/cache` | 已提交實作；Cloudflare 外部 token、DNS 與正式 VM 尚待設定 | [快取監控](cache-monitoring.md) |
...
目前 migration 包含多語系、Prompt／用量、AI 配圖、Worker 心跳、作者庫、GA4、搜尋通知、公開快取失效與 Worker desired state；最新快取相關 migration 為 `20260906130000_public_invalidations`、`20260906140000_worker_desired_state`。migration 檔存在不代表正式環境已套用。

發布前先完成適用的隔離資料庫回歸、型別檢查與 production build，確認 migration 與環境設定，再依 README 的發布政策取得確認。搜尋引擎功能應先完成上列缺口再啟用排程。

## 2026-09-06 最新驗證

[測試紀錄](test-log.md) 記錄 90 個測試檔／310 項測試通過、TypeScript 通過、Lint 通過、production build 通過。測試使用本機隔離 PostgreSQL；尚未執行正式 VM、Cloudflare 外部 purge、R2／Gemini 付費整合與瀏覽器實機效能測試。
