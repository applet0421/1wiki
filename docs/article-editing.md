...
文件狀態：現行操作文件；文章卡片、封面、發布補值及影片已有程式實作，真實 R2 上傳仍依環境驗證。

## 封面與文章卡片

首頁與分類列表的共用文章卡片在 `coverImage` 有值時顯示可點擊的延後載入圖片，沒有封面時維持文字卡片。首頁目前工作樹正改為直接重用分類樣式的 `ArticleFeedList`；完成狀態見 [工作狀態](project-status.md)。卡片封面與 YouTube Shorts 支援已提交於 `5339e2e`。
