...
export function WeChatImportWorkspace({ imported }: { imported: WeChatImportView }) {
  ...
  function edit(field: "title" | "excerpt" | "slug" | "seoTitle" | "seoDescription" | "seoKeywords", value: string) { setDraft((current) => current ? { ...current, [field]: value } : null); }
  ...
  return <div className={styles.workspace}>
    <header className={styles.heading}><div><p className="eyebrow">文章生成 / 微信公眾號改寫</p><h1>{abandoned ? "工作已放棄" : expired ? "匯入已到期" : wizardSteps[step - 1]}</h1><p className="muted">一步一步確認內容，完成後再交給文章編輯器。</p></div><div className={styles.headingActions}><span className={styles.badge} role="status">{expired && !abandoned ? "暫存期限已到" : importStatusLabels[imported.status] || "工作狀態待確認"}</span>{!ready && !unavailable && <button className={styles.dangerButton} disabled={pending} onClick={() => setConfirm("abandon")}>重置工作</button>}</div></header>
