...
export function WeChatArticlePreview({ title, blocks, assets, fallbackHtml }: { title: string; blocks: ArticleBlock[]; assets: WeChatAssetView[]; fallbackHtml?: string | null }) {
  return <article className={styles.article}>
    ...
    {blocks.length ? blocks.map((block) => {
      if (block.type === "text") return <div className="article-content" key={block.id} dangerouslySetInnerHTML={{ __html: block.html }} />;
      const asset = assets.find((item) => item.id === block.assetId);
      return <figure key={block.id}>{asset ? <WeChatPreviewImage asset={asset} alt={block.alt || "文章圖片"} /> : <p className={styles.warning}>圖片引用不存在，請勿轉存此版本。</p>}{block.alt && <figcaption>{block.alt}</figcaption>}</figure>;
