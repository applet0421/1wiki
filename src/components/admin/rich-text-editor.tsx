...
export function RichTextEditor({ initialHtml = "", inputName = "contentHtml", ariaLabel = "文章正文", onHtmlChange, aiImageContext }: RichTextEditorProps) {
  ...
  return <div className="editor-frame">
    <div className="editor-toolbar" role="toolbar" aria-label="文章格式">
      <button type="button" onClick={() => command("formatBlock", "p")}>段落</button><button type="button" onClick={() => command("formatBlock", "h2")}>H2</button><button type="button" onClick={() => command("formatBlock", "h3")}>H3</button><button type="button" onClick={() => command("bold")}><strong>粗體</strong></button><button type="button" onClick={() => command("italic")}><em>斜體</em></button><button type="button" onClick={() => command("underline")}><u>底線</u></button><button type="button" onClick={() => openInsert("link")}>連結</button><button type="button" onClick={() => openInsert("image")}>圖片網址</button><button type="button" onClick={() => openInsert("youtube")}>YouTube</button><button type="button" onClick={() => fileInputRef.current?.click()}>上傳圖片</button>{aiImageContext ? <button type="button" aria-expanded={aiImageOpen} onClick={() => setAIImageOpen(!aiImageOpen)}>AI 配圖</button> : null}<button type="button" onClick={() => command("insertUnorderedList")}>項目</button><button type="button" onClick={() => command("insertOrderedList")}>編號</button><button type="button" onClick={() => { const editor = editorRef.current; if (!editor) return; editor.focus(); restoreSelection(); document.execCommand("insertHTML", false, '<div class="article-callout article-callout-warning"><strong>注意</strong><p>請輸入需要留意的事項。</p></div>'); updateHtml(editor.innerHTML); }}>注意提示</button>
    </div>
    <div className="editor-toolbar" role="group" aria-label="文章版型"><button type="button" onClick={() => { const editor = editorRef.current; if (!editor) return; editor.focus(); restoreSelection(); document.execCommand("insertOrderedList"); const list = editor.querySelector("ol:last-of-type"); if (list) list.className = "article-steps"; updateHtml(editor.innerHTML); }}>步驟清單</button><button type="button" onClick={() => { const editor = editorRef.current; if (!editor) return; editor.focus(); restoreSelection(); document.execCommand("insertHTML", false, '<div class="article-callout article-callout-warning"><strong>注意</strong><p>請輸入需要留意的事項。</p></div>'); updateHtml(editor.innerHTML); }}>注意提示</button></div>
