...
export function RichTextEditor({ initialHtml = "" }: { initialHtml?: string }) {
  ...
  function command(name: string, value?: string) { editorRef.current?.focus(); document.execCommand(name, false, value); setHtml(editorRef.current?.innerHTML || ""); }
  ...
  return <div className="editor-frame">
    ...
    <div ref={editorRef} className="rich-editor article-prose" contentEditable suppressContentEditableWarning dangerouslySetInnerHTML={{ __html: initialHtml }} onInput={(event) => setHtml(event.currentTarget.innerHTML)} aria-label="文章正文" />
