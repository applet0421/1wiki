...
export function AIGenerator({ provider, onGenerated }: { provider: string; onGenerated: (article: GeneratedArticle) => void }) {
  ...
  async function generate() { setLoading(true); setError(""); const result = await generateArticleAction({ topic, keyword, instructions }); setLoading(false); if (!result.ok) { setError(result.error); return; } onGenerated(result.data); }
