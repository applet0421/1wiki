...
export function AIRewriter({ categories, provider }: { categories: CategoryOption[]; provider: string }) {
  ...
  async function rewrite() {
    ...
    setError("");
    try {
      const result = await rewriteArticleAction({ sourceTitle, sourceContentHtml });
      if (!result.ok) {
  ...
  return <div className="admin-grid">
    ...
    {rewritten ? <section className="rewrite-result" aria-label="AI 改寫結果">
      ...
      <PostEditor key={`${rewritten.title}-${rewritten.seoDescription}`} categories={categories} initialGenerated={rewritten} provider={provider} showAIGenerator={false} />
