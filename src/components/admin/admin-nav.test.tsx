...
describe("AdminNav", () => {
  it("shows Prompt, usage, and account management to owners in order", () => {
    ...
    expect(screen.getByRole("link", { name: "品牌與 SEO" })).toHaveAttribute("href", "/admin/brand-seo");
    expect(screen.getByRole("link", { name: "文章廣告" })).toHaveAttribute("href", "/admin/ads");
    ...
    expect(screen.getByRole("link", { name: "快取監控" })).toHaveAttribute("href", "/admin/cache");
  });
  ...
  it("hides owner-only management from editors", () => {
    ...
    expect(screen.queryByRole("link", { name: "品牌與 SEO" })).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "文章廣告" })).not.toBeInTheDocument();
    ...
    expect(screen.queryByRole("link", { name: "快取監控" })).not.toBeInTheDocument();
