...
export async function ArticlePanel({ post, locale, initial = false }: { post: ArticlePost; locale: Locale; initial?: boolean }) {
  ...
  return <><AdsenseScript clientId={clientId} /><section className="article-layout" aria-label={post.title}><article className="article-page">{initial ? <><JsonLd value={buildArticleJsonLd(post, getSiteUrl(), locale)} /><JsonLd value={buildBreadcrumbJsonLd(breadcrumbItems, getSiteUrl())} /></> : null}<CategoryBreadcrumbs ancestors={ancestors} current={post.category} articleTitle={post.title} locale={locale} /><header className="article-header"><p className="eyebrow">{post.category.name}</p>{initial ? <h1>{post.title}</h1> : <h2 className="article-title"><Link href={pathname}>{post.title}</Link></h2>}<p className="article-excerpt">{post.excerpt}</p><div className="article-meta"><span>{post.author.displayName}</span><time dateTime={post.publishedAt?.toISOString()}>{post.publishedAt ? new Intl.DateTimeFormat(dateLocale, { dateStyle: "long" }).format(post.publishedAt) : ""}</time>{post.updatedAt > (post.publishedAt || post.createdAt) ? <span>{dictionary.article.updated} {new Intl.DateTimeFormat(dateLocale, { dateStyle: "medium" }).format(post.updatedAt)}</span> : null}</div></header><ArticleBody html={post.contentHtml} pathname={pathname} adEnvironment={adEnvironment} />{relatedPosts.length > 0 ? <aside className="related-box" aria-label={dictionary.article.relatedTitle}>
