...
export async function CategoryPageContent({
  ...
  const adSettings = await getOrCreateArticleAdSettings(prisma);
  const categoryPlacements = ["category_inline", "category_end", "category_sidebar_desktop"] as const;
  const hasLiveSlot = showAds && categoryPlacements.some((placement) => getAdSlotConfig(placement, adEnvironment, adContext)?.mode === "live");
  const anchorConfig = getAnchorAdsConfig(adSettings, "category");
  const clientId = hasLiveSlot || anchorConfig.enabled ? getLiveAdsenseClientId(adEnvironment, pathname) : null;

  return (
    <main className="public-main">
      <AdsenseScript clientId={clientId} enableBottomAnchor={anchorConfig.enabled} />
      ...
      <div className="category-content-layout">
        <section aria-label="文章列表">
          <CategoryArticleList initialPosts={data.posts} locale={locale} dictionary={dictionary} path={segments.join("/")} adInterval={adSettings.categoryInlineAdInterval} inlineAdConfig={getAdSlotConfig("category_inline", adEnvironment, adContext)} />
