...
export default async function HomePage({ params }: Props) {
  ...
  return <><AdsenseScript clientId={clientId} enableBottomAnchor={anchorConfig.enabled} /><main className="public-main"><section className="home-hero"><div><p className="eyebrow">{dictionary.home.eyebrow}</p><h1>{dictionary.home.title}</h1><p>{dictionary.home.intro}</p></div></section><section><div className="section-title"><p className="eyebrow">{dictionary.home.exploreEyebrow}</p><h2>{dictionary.home.exploreTitle}</h2></div><div className="category-grid">{categories.map((category, index) => <Link href={getCategoryHref(locale, [category.slug])} className="category-card" key={category.id}><span>{String(index + 1).padStart(2, "0")}</span><h3>{category.name}</h3><p>{category.description}</p><strong>{dictionary.home.viewCategory}</strong></Link>)}</div></section><section><div className="section-title"><p className="eyebrow">{dictionary.home.latestEyebrow}</p><h2>{dictionary.home.latestTitle}</h2></div><div className="article-list" data-testid="latest-answers">{posts.map((post) => <ArticleCard key={post.id} post={post} locale={locale} dictionary={dictionary} />)}</div></section></main></>;
}
