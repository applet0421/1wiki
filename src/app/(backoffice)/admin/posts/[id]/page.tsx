...
export default async function EditPostPage({ params, searchParams }: Props) {
  ...
  const [post, categories] = await Promise.all([prisma.post.findUnique({ where: { id } }), listCategories(prisma)]);
  if (!post) notFound();
  ...
  return <section><p className="eyebrow">編輯內容</p><h1>{post.title}</h1>{query.success === "generated" ? <p className="form-success">AI 草稿已建立，請檢查內容後再發布。</p> : null}<PostEditor locale={post.locale as Locale} categories={options} post={post} error={query.error} provider={process.env.LLM_PROVIDER || "deepseek"} /></section>;
}
