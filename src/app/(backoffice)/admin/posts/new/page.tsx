...
export default async function NewPostPage({ searchParams }: Props) {
  const [categories, params, authors] = await Promise.all([listCategories(prisma), searchParams, listAuthorOptions(prisma)]);
  ...
  return <section><p className="eyebrow">新增內容</p><h1>建立文章</h1><PostEditor authors={authors} locale={locale} categories={options} error={params.error} provider={process.env.LLM_PROVIDER || "deepseek"} /></section>;
}
