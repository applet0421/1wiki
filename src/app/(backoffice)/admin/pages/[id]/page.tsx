...
import { getSitePage } from "@/lib/content/pages";
...
export default async function EditSitePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const page = await getSitePage(prisma, id);
  if (!page || !isLocale(page.locale)) notFound();
  return <section><p className="eyebrow">網站內容</p><h1>編輯網站頁面</h1><SitePageEditor page={page} locale={page.locale} error={query.error} /></section>;
}
