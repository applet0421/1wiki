...
export default async function SitePagesPage({ searchParams }: { searchParams: Promise<{ locale?: string; error?: string; success?: string }> }) {
  ...
  return <section className="admin-grid">
    ...
    <div className="panel table-wrap">{pages.length === 0 ? <p className="muted">尚未建立網站頁面。</p> : <table><thead><tr><th>頁面</th><th>語系</th><th>分類</th><th>狀態</th><th>更新時間</th><th>操作</th></tr></thead><tbody>{pages.map((page) => <tr key={page.id}><td><strong>{page.title}</strong><small>/{page.slug}</small></td><td>{isLocale(page.locale) ? getLocaleConfig(page.locale).label : page.locale}</td><td>{page.category?.name || "未掛載"}</td><td><span className={`status status-${page.status.toLowerCase()}`}>{page.status === "PUBLISHED" ? "已發布" : "草稿"}</span></td><td>{page.updatedAt.toLocaleDateString("zh-TW")}</td><td><div className="row-actions"><Link className="button button-quiet" href={`/admin/pages/${page.id}`}>編輯</Link>{page.status === "PUBLISHED" ? <Link className="button button-quiet" href={`/${page.locale}/${page.slug}`} target="_blank" rel="noreferrer">查看</Link> : null}<form action={toggleSitePageStatusAction}><input type="hidden" name="id" value={page.id} /><input type="hidden" name="status" value={page.status === "PUBLISHED" ? "DRAFT" : "PUBLISHED"} /><button className="button button-quiet" type="submit">{page.status === "PUBLISHED" ? "撤回" : "發布"}</button></form><form action={deleteSitePageAction}><input type="hidden" name="id" value={page.id} /><button className="button button-danger" type="submit">刪除</button></form></div></td></tr>)}</tbody></table>}</div>
  </section>;
}
