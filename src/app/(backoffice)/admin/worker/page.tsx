...
export default async function WorkerPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [heartbeat, searchHeartbeat, wechatHeartbeat, grouped, wechatGrouped, totalJobs, recent, searchSummary] = await Promise.all([
    ...
    workerHeartbeat?.findUnique({ where: { id: "wechat-import-worker" } }) ?? Promise.resolve(null),
    prisma.imageGeneration.groupBy({ by: ["status"], _count: { _all: true } }),
    prisma.weChatImport.groupBy({ by: ["status"], _count: { _all: true } }),
  ...
  const wechatCounts = Object.fromEntries(wechatGrouped.map((item) => [item.status, item._count._all]));
  const imageStatus = heartbeat?.desiredState === "STOPPED" ? { label: "已停用", className: "status-draft" } : statusFor(heartbeat?.lastHeartbeat);
  const searchStatus = searchHeartbeat?.desiredState === "STOPPED" ? { label: "已停用", className: "status-draft" } : statusFor(searchHeartbeat?.lastHeartbeat, 10 * 60_000, 30 * 60_000);
  const wechatStatus = wechatHeartbeat?.desiredState === "STOPPED" ? { label: "已停用", className: "status-draft" } : statusFor(wechatHeartbeat?.lastHeartbeat);
  ...
  return <section className="admin-grid">
    ...
    <div className="panel"><div className="heading-row"><div><h2>微信匯入 Worker</h2><p className="muted">擷取公開微信文章、執行 LLM 改寫並在確認後轉存圖片至 R2。</p><p className="muted">最近心跳：{time(wechatHeartbeat?.lastHeartbeat)} · 已處理：{wechatHeartbeat?.processed || 0} · 控制狀態：{wechatHeartbeat?.desiredState || "—"}</p></div><span className={`status ${wechatStatus.className}`}>{wechatStatus.label}</span></div><div className="worker-toolbar"><form method="get"><button className="button button-quiet" type="submit">重新整理</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="wechat-import" /><input type="hidden" name="action" value="start" /><button className="button button-primary" type="submit">設定啟用</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="wechat-import" /><input type="hidden" name="action" value="restart" /><button className="button button-quiet" type="submit">要求重啟</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="wechat-import" /><input type="hidden" name="action" value="stop" /><button className="button button-quiet" type="submit">設定停用</button></form></div><div className="metric-grid"><div className="metric-card"><span>待擷取</span><strong>{wechatCounts.FETCH_QUEUED || 0}</strong></div><div className="metric-card"><span>待改寫</span><strong>{wechatCounts.REWRITE_QUEUED || 0}</strong></div><div className="metric-card"><span>待轉存</span><strong>{wechatCounts.TRANSFER_QUEUED || 0}</strong></div><div className="metric-card"><span>需處理失敗</span><strong>{(wechatCounts.FAILED || 0) + (wechatCounts.UNKNOWN || 0) + (wechatCounts.TRANSFER_FAILED || 0)}</strong></div></div>{wechatHeartbeat?.lastError ? <p className="form-error" role="alert">微信匯入 Worker 最近錯誤：{wechatHeartbeat.lastError}</p> : null}</div>
    <div className="panel"><div className="heading-row"><div><h2>搜尋引擎通知 Worker</h2><p className="muted">由 Cron 定期執行 Bing IndexNow 與 Google sitemap 提交。</p><p className="muted">最後心跳：{time(searchHeartbeat?.lastHeartbeat)} · 已處理：{searchHeartbeat?.processed || 0} · 控制狀態：{searchHeartbeat?.desiredState || "—"}</p></div><span className={`status ${searchStatus.className}`}>{searchStatus.label}</span></div><div className="worker-toolbar"><form method="get"><button className="button button-quiet" type="submit">重新整理</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="search-engine" /><input type="hidden" name="action" value="start" /><button className="button button-primary" type="submit">設定啟用</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="search-engine" /><input type="hidden" name="action" value="restart" /><button className="button button-quiet" type="submit">要求重啟</button></form><form action={controlWorkerAction}><input type="hidden" name="worker" value="search-engine" /><input type="hidden" name="action" value="stop" /><button className="button button-quiet" type="submit">設定停用</button></form></div><div className="metric-grid"><div className="metric-card"><span>Bing 待處理</span><strong>{searchSummary.pending}</strong></div><div className="metric-card"><span>Bing 失敗</span><strong>{searchSummary.failed}</strong></div><div className="metric-card"><span>GSC 待處理</span><strong>{searchSummary.google.pending}</strong></div><div className="metric-card"><span>GSC 失敗</span><strong>{searchSummary.google.failed}</strong></div></div>{searchHeartbeat?.lastError ? <p className="form-error" role="alert">搜尋引擎 Worker 最近錯誤：{searchHeartbeat.lastError}</p> : null}</div>
