...
export default async function SearchEnginePage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <section className="admin-grid">
    ...
    <div className="metric-grid"><div className="metric-card"><span>Sitemap</span><strong>可用</strong></div><div className="metric-card"><span>待處理通知</span><strong>{summary.pending}</strong></div><div className="metric-card"><span>成功</span><strong>{summary.success}</strong></div><div className="metric-card"><span>失敗</span><strong>{summary.failed}</strong></div></div>
    <div className="panel"><div className="heading-row"><div><h2>自動通知</h2><p className="muted">發布、更新或下架文章時會自動加入 Bing IndexNow 佇列。</p><p className="muted">Sitemap：{siteUrl}/sitemap.xml</p></div><form action={processSearchEngineAction}><button className="button button-primary">立即處理通知</button></form></div></div>
