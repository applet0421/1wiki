...
export default async function EditPostPage({ params, searchParams }: Props) {
  ...
  if (!post) notFound();
  return <section><p className="eyebrow">編輯內容</p><h1>{post.title}</h1>{query.success === "generated" ? <p className="form-success">AI 草稿已建立，請檢查內容後再發布。</p> : null}<PostEditor categories={categories} post={post} error={query.error} provider={process.env.LLM_PROVIDER || "deepseek"} /></section>;
}
