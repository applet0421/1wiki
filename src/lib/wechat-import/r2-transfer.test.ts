...
describe("WeChat R2 transfer", () => {
  ...
  it("uploads staged assets then immediately removes their database bytes", async () => {
    ...
    const asset = await prisma.weChatImportAsset.findFirstOrThrow({ where: { importId: imported.id } });
    await prisma.weChatImport.update({ where: { id: imported.id }, data: { rewrittenDraft: { title: "改寫標題", slug: "rewritten-title", excerpt: "摘要", seoTitle: "改寫標題", seoDescription: "摘要", seoKeywords: "關鍵字", needsVerification: [], blocks: [{ id: "b-0001", type: "text", html: "<p>改寫段落</p>" }, { id: "b-0002", type: "image", assetId: asset.id, alt: "LINE 同步畫面" }] } } });
    ...
    await expect(prisma.weChatImport.findUniqueOrThrow({ where: { id: imported.id }, include: { assets: true } })).resolves.toMatchObject({ status: "READY", editorDraft: expect.objectContaining({ sourceImportId: imported.id, title: "改寫標題", contentHtml: expect.stringContaining('<figure class="article-image article-image-standard"><img') }), assets: [{ status: "READY", imageBytes: null, publicUrl: expect.stringContaining("https://images.example.com/") }] });
  });
});
