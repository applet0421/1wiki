...
export async function createWeChatImport(client: PrismaClient, userId: string, input: { sourceUrl: string; targetLocale: string }, now = new Date()) {
  ...
  const imported = await client.weChatImport.create({ data: { userId, sourceUrl: input.sourceUrl.trim(), normalizedUrl, targetLocale: input.targetLocale, expiresAt: new Date(now.getTime() + 24 * 60 * 60 * 1000) } });
